...
class SubTrabalho extends CI_Controller {
    ...
    public function getNomeGtPorId($id_gt) {
      ...
      public function inserirGT() {
        $data['id_evento'] = $this->input->post('id_evento');
        $data['descricao'] = $this->input->post('descricao');
        $data['sigla_gt'] = $this->input->post('sigla');
        
        $id_evento = $this->input->post('id_evento');
        $idArea = 4;
        $nomeArea = $this->input->post('descricao');
        $this->subTrabalho->inserirGT($data);
        
        $data2['id_gt'] = $this->subTrabalho->retornaLastInsert();
        
        foreach ($data2['id_gt'] as $dados) {
            $id = $dados->id_gt_evento;
        }
        
        echo "         
                                    <tbody id='area-$id'>
                                    <td style='text-align: left'>$nomeArea</td>
                                    <td>
                                        <button class='btn btn-info btn-xs'  data-toggle='tooltip' data-placement='bottom' title='Editar' onclick=\"editarArea('$id');\"><i class='glyphicon glyphicon-edit'></i></button>
                                        <button class='btn btn-danger btn-xs'  data-toggle='tooltip' data-placement='bottom' title='Apagar' onclick=\"deletarArea('$id');\"><i class='glyphicon glyphicon-remove'></i></button>
                                        <button class='btn btn-success btn-xs'
                                                data-toggleArea='tooltipArea' data-placement='bottom' title='Avaliadores'       
                                                data-toggle='modal' data-target='#avaliadores'                                                                                                                                         
                                                onclick=\"buscarAvaliadores('$id_evento', '$idArea');\"
    ...
    public function deletarArea() {
        ...
        $this->subTrabalho->deletarArea($id);
    }
